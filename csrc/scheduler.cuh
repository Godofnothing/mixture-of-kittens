...
template <int WORLD_SIZE_>
struct config {
    // Keep the active peer count compile-time so all peer scans are unrolled.
    static constexpr int WORLD_SIZE = WORLD_SIZE_;
...
static __device__ __forceinline__ void count_kernel(const globals &G) {
    constexpr int world_size = config<WORLD_SIZE>::WORLD_SIZE;
...
static __device__ __forceinline__ void pad_kernel(const globals &G) {
    using kernel_config = config<WORLD_SIZE>;
    const int local_expert = blockIdx.x;
    constexpr int world_size = kernel_config::WORLD_SIZE;
    int num_tokens = 0;
    #pragma unroll
    ...
    const int padded_num_tokens = (num_tokens + kernel_config::EXPERT_PADDING - 1) / kernel_config::EXPERT_PADDING * kernel_config::EXPERT_PADDING;
...
static __device__ __forceinline__ void schedule_kernel(const globals &G) {
    ...
    __shared__ int cumulative_tokens_from_peer_rank[kernel_config::NUM_WARPS];
    ...
    for (int idx = blockIdx.x; idx < num_local_experts * world_size; idx += gridDim.x) {
        ...
        if (warpid() == 0) {
            int warp_total = (warp::laneid() < kernel_config::NUM_WARPS) ? cumulative_tokens_from_peer_rank[warp::laneid()] : 0;
            for (int offset = 1; offset < WARP_THREADS; offset *= 2) {
                const int n = __shfl_up_sync(0xffffffff, warp_total, offset);
                if (warp::laneid() >= offset) warp_total += n;
            }
            if (warp::laneid() < kernel_config::NUM_WARPS) cumulative_tokens_from_peer_rank[warp::laneid()] = warp_total;
        }
        __syncthreads();
...
template <int WORLD_SIZE>
static __host__ std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> schedule_impl(
    ...
    kittens::py::global_kernel<kernel_config, globals, scheduler::count_kernel<WORLD_SIZE>>
        <<<(G.topk.numel() + kernel_config::NUM_THREADS - 1) / kernel_config::NUM_THREADS, kernel_config::NUM_THREADS, num_local_experts * world_size * sizeof(int), stream>>>(G);
    kittens::py::global_kernel<kernel_config, globals, scheduler::pad_kernel<WORLD_SIZE>>
        <<<num_local_experts, 1, 0, stream>>>(G);
    kittens::py::global_kernel<kernel_config, globals, scheduler::schedule_kernel<WORLD_SIZE>>
        <<<num_local_experts * world_size, kernel_config::NUM_THREADS, world_size * sizeof(int), stream>>>(G);
...
static __host__ std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor> schedule(
    ...
    switch (topk_all.size(0)) {
        case 1: return schedule_impl<1>(topk_all, num_local_experts, schedule_capacity, rank);
        case 2: return schedule_impl<2>(topk_all, num_local_experts, schedule_capacity, rank);
        case 4: return schedule_impl<4>(topk_all, num_local_experts, schedule_capacity, rank);
        case 8: return schedule_impl<8>(topk_all, num_local_experts, schedule_capacity, rank);
        default: throw std::runtime_error("MoK scheduler requires intra-host EP size 1, 2, 4, or 8");
    }
}
